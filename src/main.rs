...
impl LogicExp {
    fn parse(sexp: Sexp) -> Result<Self, Error> {
        ...
        match sexp {
            Sexp::Atom(Atom::S(s)) => Ok(LogicExp::Variable(String::from(s))),
    ...
    fn solve(&self, map: &HashMap<String, bool>) -> bool {
        match self {
            ...
            LogicExp::Iff(e0, e1) => ((!e0.solve(map)) | e1.solve(map)) & ((!e1.solve(map)) | e0.solve(map)),
        }
    }
    ...
    fn print_latex(&self) -> String {
        match self {
            ...
            LogicExp::Implies(e0, e1) => format!("({} \\rightarrow {})", e0.print_latex(), e1.print_latex()),
    ...
    fn find_vars(&self) -> HashSet<String> {
        match self {
            ...
            LogicExp::And(e0, e1) => e0.find_vars().union(&e1.find_vars()).map(|x| x.clone()).collect(),
            LogicExp::Or(e0, e1) => e0.find_vars().union(&e1.find_vars()).map(|x| x.clone()).collect(),
            LogicExp::Implies(e0, e1) => e0.find_vars().union(&e1.find_vars()).map(|x| x.clone()).collect(),
            LogicExp::Iff(e0, e1) => e0.find_vars().union(&e1.find_vars()).map(|x| x.clone()).collect(),
        }
    }
    ...
    fn simple_table(&self) -> String {
        ...
        for i in 0..num_vars {
           fmt_s.push_str("L|");
           header_s.push_str(&format!(" {} &", vars[i]));
        }
    ...
    fn get_steps(&self) -> Vec<Self> {
        let mut prev = match self {
            ...
            LogicExp::And(e0, e1) | LogicExp::Or(e0, e1) | LogicExp::Implies(e0, e1) | LogicExp::Iff(e0, e1) => {
